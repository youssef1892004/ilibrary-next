...
// 1. استعلام GraphQL لجلب قائمة الكتاب
// سنقوم بترتيبهم حسب عدد الكتب لعرض الأبرز منهم
const GET_LATEST_WRITERS = gql`
  query GetLatestWriters {
    ilibarary_Autor(limit: 8, order_by: { book_num: desc }) {
      id
      name
      image_url
      bio
...
const LatestWriters = () => {
  const { t } = useLanguage();
  // 2. استخدام useQuery لجلب بيانات الكتاب من الـ API
  ...
  // 3. استخلاص قائمة الكتاب من البيانات
    ...
    <section id="latest-writers" className="py-16 bg-gray-50 dark:bg-gray-900">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* 4. عرض بطاقات الكتاب القادمين من الـ API */}
          {writers.map((writer) => (
            <div key={writer.id} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg text-center transition-transform duration-300 hover:-translate-y-2">
              <div className="relative w-32 h-32 mx-auto mb-4">
                <img
                  ...
                  alt={writer.name}
                  onError={(e) => { e.target.onerror = null; e.target.src=`https://ui-avatars.com/api/?name=${encodeURIComponent(writer.name)}&background=random`; }}
                />
              </div>
              ...
              <Link href={`/writers/${writer.id}`}>
                <button className="mt-4 px-6 py-2 bg-purple-100 text-purple-700 dark:bg-purple-800 dark:text-purple-200 font-semibold rounded-lg hover:bg-purple-200 dark:hover:bg-purple-700 transition-colors">
